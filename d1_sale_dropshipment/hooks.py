...
Zelfde patroon als d1_sale_order_checks: op een verse database gebeurt er
niets; op de productiedatabase wordt de data gekopieerd en worden de
vervangen Studio-automations, -velden en view-verwijzingen opgeruimd.
"""
import logging
...
MODULE = "d1_sale_dropshipment"

COLUMN_COPIES = [
    ("res_partner", "x_studio_dropshipment", "d1_dropshipment",
     "x_studio_dropshipment"),
    ("sale_order", "x_studio_dropshipment", "d1_dropshipment",
     "x_studio_dropshipment"),
]

REPLACED_AUTOMATIONS = [
    ("sale.order", "Verkooporder: Dropshipment overnemen van klantgegevens"),
]

REPLACED_FIELDS = [
    ("res.partner", "x_studio_dropshipment"),
    ("sale.order", "x_studio_dropshipment"),
]
...
def _copy_columns(env):
    ...
    for table, src, dest, expr in COLUMN_COPIES:
        ...
        env.cr.execute(
            ...
            "WHERE {src} IS NOT NULL AND {dest} IS DISTINCT FROM {expr}".format(
...
def _remove_replaced_automations(env):
    ...
    for model_name, name in REPLACED_AUTOMATIONS:
        ...
        _logger.info("%s: removed automation '%s' (%s, %s records)",
                     MODULE, name, model_name, len(autos))
...
def _remove_replaced_fields(env):
    ...
    Faalt een unlink (veld nog ergens in gebruik), dan wordt dat gelogd en
    overgeslagen zodat de installatie nooit blokkeert.
    """

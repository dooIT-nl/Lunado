...
class DmApi:
    ...
    def reqeust_book_shipment(
            self, customer: Customer, shipment: Shipment, products: DmProducts,
    ):
        try:
    ...
    def request_shipping_options(
        ...
        try:
            ...
            if not Helper.is_empty(shipment.delivery_date):
                body_shipment["deliveryDate"] = shipment.delivery_date
        ...
        try:
            ...
            if "status" in response and "code" in response and "message" in response:
                if (
                        ...
                        and response["code"] != 7
                        and shipment_to_hub == False
                ):
                    self._logger.error(response["message"])
                    self._logger.error(f"DeliveryMatch API response: {str(response)}")
                    raise DeliveryMatchException(
                        f"Invalid API response from DeliveryMatch server: {response['message']}"
                    )

                if (
                        response["status"] != "failure"
                        and response["code"] != 72
                        and shipment_to_hub == True
                ):
                    self._logger.error(response["message"])
                    self._logger.error(f"DeliveryMatch API response: {str(response)}")
                    raise DeliveryMatchException(
            ...
            raise Exception(
                "Something went wrong in the validation of the booking response"
            )

...
class DmApi:
    ...
    def set_request_url(self, shipment_id, get_shipment_response=None) -> str:
        ...
        if Helper.is_empty(get_shipment_response) or 'shipment' not in get_shipment_response:
            return request_url

        response_shipment_id = get_shipment_response.get("shipment").get("shipmentID")

        if Helper.is_empty(shipment_id) and Helper.is_empty(response_shipment_id):
            return request_url


        if not Helper.is_empty(shipment_id) or not Helper.is_empty(response_shipment_id):
    ...
    def reqeust_book_shipment(
        ...
        try:
            ...
            get_shipment_response = self.get_shipment(id=shipment.id, order_number=odoo_order_number)
            shipment_id = get_shipment_response.get('shipment', {}).get('shipmentID') or shipment.id
    ...
    def validate_booking_response(self, response, shipment_to_hub=False):
        try:
            ...
            if("status" not in response_obj or response_obj["status"] is not "success"):
                raise DeliveryMatchException(
                    f"Invalid API response from DeliveryMatch server: {response.text}"
                )

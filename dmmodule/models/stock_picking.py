...
class StockPicking(models.Model):
    ...
    hide_book_carrier_btn = fields.Boolean(default=False)
    dm_is_external_warehouse = fields.Boolean(default=False)
    ...
    def show_shipping_options(self):
        try:     
            ...
            if(self.is_delivery() == True):
                sale_order_number = self.sale_id.display_name
                
            # if delivery order is inbound
            if(self.is_delivery() == False):
                pruchase_order = self.get_purchase_order("name", self.origin)
                
                if(pruchase_order == None or pruchase_order == False):
                    raise DeliveryMatchException("Given purchase order not found")
                
                if(pruchase_order.origin == None or pruchase_order.origin == False):
                    raise DeliveryMatchException("In order to add an inbound order in DeliveryMatch please provide an ordernumber in the purchase order -> origin field.")
                
                sale_order_number = pruchase_order.origin

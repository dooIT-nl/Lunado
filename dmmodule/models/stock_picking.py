...
class StockPicking(models.Model):
    ...
    def create(self, vals):
        ...
        res.dm_is_external_warehouse = res.location_id.warehouse_id.dm_external_warehouse
        res.hide_book_carrier_btn = res.get_hide_book_carrier_btn()
        return res
    
    def write(self, values):
        
        if(self.picking_type_id.dm_is_inbound == True and 'location_dest_id' in values):
            db_location_dest_id = self.env['stock.location'].search([('id', '=', values['location_dest_id'])], limit=1)
            if db_location_dest_id:
                values['dm_is_external_warehouse'] = db_location_dest_id.warehouse_id.dm_external_warehouse
            else:
                values['dm_is_external_warehouse'] = False
        
        if(self.picking_type_id.dm_is_inbound == False and 'location_id' in values):
            values['dm_is_external_warehouse'] = self.location_id.warehouse_id.dm_external_warehouse
        
        values['hide_book_carrier_btn'] = self.get_hide_book_carrier_btn()
    ...
    def get_products_details(self) -> DmProducts:
        try:
            ...
            for ol in self._origin.move_ids_without_package:
                ...
                if(hasattr(ol, 'x_studio_hoeveelheid') == True):
                    quantity = ol.x_studio_hoeveelheid
                else:
                    quantity = ol.product_uom_qty
                    
                if(product_line.detailed_type != "product" or quantity <= 0): continue
                
                in_stock: bool = (ol.product_qty_available < quantity)
                ...
                if (has_x_studio_length == True and self.override_product_length() == True):
                    ...
                    if order_line.x_studio_length > 0:
                        length = order_line.x_studio_length * 100

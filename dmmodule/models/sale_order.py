...
class SaleOrder(models.Model):
    ...
    def get_shipment_details(self):
        ...
        shipment_id = DmApi(self.get_base_url(), self.get_api_key(), self.get_client_id()).get_shipment_by_ordernumber(self.display_name, True)   
        if(shipment_id == None or shipment_id == "" or shipment_id == False):
            shipment_id = False
    ...
    def get_products_details(self) -> DmProducts:
        try:
            ...
            for ol in self.order_line:
                ...
                if(hasattr(ol, 'x_studio_qty') == True and hasattr(ol, 'x_studio_length') == True and override_length == True):
                    if(ol.x_studio_length > 0): 
                        length = ol.x_studio_length * 100
                        quantity = ol.x_studio_qty # Maatwerk Lunado Hoeveelheid
                        
                  
                in_stock: bool = (product_line.qty_available < quantity)
        
                if(product_line.dm_send_lot_code == True): custom1 = Helper.remove_letters_from_str(self.display_name)

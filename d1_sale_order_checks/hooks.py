...
def _remove_replaced_fields(env):
    ...
    Als een veld nog ergens in gebruik is (bv. een niet-opgeschoonde view),
    laat Odoo de unlink falen; dat wordt gelogd en overgeslagen zodat de
    installatie nooit blokkeert — opschonen kan dan handmatig.
    """
    for model_name, field_name in REPLACED_FIELDS:
        ...
        try:
            field.unlink()
            _logger.info(
                "d1_sale_order_checks: removed studio field %s.%s",
                model_name, field_name,
            )
        except Exception:
            _logger.warning(
                "d1_sale_order_checks: could not remove studio field %s.%s "
                "(still referenced?); please clean up manually",
